...
import { notePathApi, renameNotePathApi, shareNotePathApi, sharedNotePathApi, starNotePathApi } from './notesApi'
...
describe('notes api path helpers', () => {
  ...
  test('adds collection and mode query parameters in a stable order', () => {
    expect(shareNotePathApi('plan', 'work', 'edit')).toBe('/client-api/notes/plan/share?collection=work&mode=edit')
  ...
  test('encodes share tokens in shared note urls', () => {
    expect(sharedNotePathApi('token/with spaces')).toBe('/client-api/share/token%2Fwith%20spaces/note')
  })
})
